...
```
/novelist:auto 5万字 ダークファンタジー
```

要件(文字数など必要最低限)を伝えると、方向性の異なる**作品案を10案**提示。番号で選ぶと、設定構築 → 章立て → 全章執筆 → 納品ファイル生成(`build/<タイトル>.md`)まで自動で進む。ユーザーの関与は要件と案の選択の2回だけ。品質パイプライン(パック生成・自動lint・監査3体・canon-sync)は手動モードと同一で省略されない。中断しても再実行すれば続きから再開する。
