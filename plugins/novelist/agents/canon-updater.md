...
- **保留項目を自動確定しない。** `# PENDING` コメントが付いた項目、値に「(仮)」を含む項目(仮題など)は、本文に対応する記述があっても canon に確定登録せず「要人間判断」に回す。ユーザーが明示的に保留した決定を、AI が代行してはならない(実例: outline の仮題を原稿の H1 経由で本題に昇格させてしまった事故)
