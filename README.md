...
| 2 | hook + lint スクリプト(保存のたび必ず発火) | 表記ゆれ・禁止語彙・属性違反・未登録固有名詞・死亡キャラ発話・読点過多・文末単調・挿絵アンカー消失 | **ゼロ** |
| 3 | 監査エージェント4体(執筆者とは独立) | 文脈依存の矛盾・口調・視点・伏線・**場面の因果と感情の温度(説得力監査)** | 高 |

実運用1作品(短編1.5万字)の検証で、設定矛盾・時系列破綻・伏線の取りこぼしはゼロで完走。その際に人間の読者だけが検出した違和感(台詞の決め台詞癖・読点過多・因果の不成立など)は、writer への規則焼き込み・定量 lint・説得力監査・learn-style として還元済み。
...
/novelist:status      # 進捗・伏線期限・sync忘れ・文字数実測の診断
/novelist:check 3     # 既存章の監査だけ実行(執筆なし)
/novelist:compile     # 全章を1つの納品ファイルに結合
/novelist:illustrate  # 挿絵プラン(シーン選定→承認→設定画の2段階)
/novelist:learn-style # AI初稿とユーザー改稿の diff から文体規則を抽出
```

### 文体を学習させる(/novelist:learn-style)

AI の初稿(canon-sync 時に `state/reference/` へ自動退避)とユーザーが手を入れた版の差分から、文体規則を実例付きで抽出して `canon/style.yaml` に登録する。**1章分の実編集はどんなヒアリングより正確な文体仕様**になる — 実運用ではこの手法が最も writer の出力を改善した。同型3例以上のみ規則化・反例確認・ユーザー承認後に登録、という規律で過剰一般化を防ぐ。
...
## 外部エディタ(note 等)と往復する場合の注意

note のエディタは見出しの `#` や場面区切りの `---` をブロック要素として保持するため、**テキストコピーで貼り戻すと記号が消える**。外部エディタで編集した本文をファイルに戻すときは:
1. 貼り戻し直後に見出し・場面区切りの有無を確認する(消えていたら復元してから保存)
2. 貼り戻しの前に、現在のファイル版との diff を取り「あなたの編集版に含まれていない最近の修正」がないか確認する(並行編集で「正」が分岐する事故の防止)
